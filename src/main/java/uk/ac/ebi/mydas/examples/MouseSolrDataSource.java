 ...
 *
 */
public class MouseSolrDataSource implements RangeHandlingAnnotationDataSource{
	CacheManager cacheManager = null;
	ServletContext svCon;
	Map<String, PropertyType> globalParameters;
	DataSourceConfiguration config;
	MouseSolrManager solrManager;
	...
	public void init(ServletContext servletContext,
		...
		try {
			...
			solrManager=new MouseSolrManager(solrUrl);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public DasAnnotatedSegment getFeatures(String segmentId, int start, int stop, Integer maxbins) 
		...
		if (maxbins==null)
			maxbins=-1;
		return solrManager.getSubmodelBySegmentId(segmentId, start, stop,maxbins);
	}
	public DasAnnotatedSegment getFeatures(String segmentId, Integer maxbins) 
			throws BadReferenceObjectException, DataSourceException {
		if (maxbins==null)
			maxbins=-1;
		return solrManager.getSubmodelBySegmentId(segmentId, -1, -1,maxbins);
	}
    ...
    public String getEntryPointVersion() throws UnimplementedFeatureException, DataSourceException {
        return solrManager.getDatabase();
    }
    ...
    public Integer getTotalCountForType(DasType type)
			throws DataSourceException {
		return solrManager.getTotalCountForType(type.getId());
	}
